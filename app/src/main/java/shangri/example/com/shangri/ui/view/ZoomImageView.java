...
public class ZoomImageView extends android.support.v7.widget.AppCompatImageView implements OnGlobalLayoutListener,
    ...
    public ZoomImageView(final Context context, AttributeSet attrs,
                         int defStyle, final int flag, final String reportHead) {
    ...
    // flag,默认为0，执行保存图片操作，flag=1代表取消长按效果,2代表举报+保存
    public ZoomImageView(Context context, Activity activity, int flag,
                         String reportHead) {
        this(context, null, 0, flag, reportHead);
        this.setDrawingCacheEnabled(true);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                // 图片不完整显示，都不需要被viewpager拦截，也就是放大情况，不能滑动到下一章图片
                ...
                break;
            case MotionEvent.ACTION_MOVE:// +0.01 防止一点点的误差导致viewpage花不到下一页
                if (rectF.width() > getWidth() + 0.01
                        || rectF.height() > getHeight() + 0.01) {
                    // 请求父控件不要拦截事件
                    // if (getParent() instanceof ViewPager) {
